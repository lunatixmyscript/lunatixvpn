...
wget -q -O /luna/run "${REPO}Fls/limit.sh && chmod +x limit.sh && ./limit.sh"
...
chmod +x /luna/run/loop-xray
clear

print_install "install Cron limit-ssh"
wget -q -O /luna/run/limit-ssh "${REPO}Fls/limit-ssh"
...
ExecStart=/luna/run/limit-xray vmip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/luna/run/limit-xray vlip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/luna/run/limit-xray trip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
